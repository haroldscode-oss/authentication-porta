...
interface LoginCardProps {
  onEmailCheck: (email: string, exists: boolean) => void
}

export function LoginCard({ onEmailCheck }: LoginCardProps) {
  const [email, setEmail] = useState("")
  const [isLoading, setIsLoading] = useState(false)
  const { checkEmail, getOAuthUrl } = useAuth()
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onEmailCheck(result.email, result.exists)
    } catch (error) {
    ...
    <div className="w-full max-w-md mx-auto">
      <div className="rounded-2xl p-8 transition-all duration-300" style={{
        ...
        <form onSubmit={handleEmailSubmit} className="space-y-4">
          ...
          >
            {isLoading ? "Checking..." : "Log in with Email"}
          </Button>
        </form>
